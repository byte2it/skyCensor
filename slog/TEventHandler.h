...
#ifndef SLOG_TEVENTHANDLER_H_
#define SLOG_TEVENTHANDLER_H_

#include <windows.h>

namespace TEventHandlerNamespace {
// Generic event handler template class
// (especially useful (but not limited to) for non-ATL clients).
...
class TEventHandler : IDispatch {
 ...
 public :
    ...
    ~TEventHandler() {
        // Call ShutdownConnectionPoint() here JUST IN CASE connection
        // points are still
    ...
    STDMETHOD(QueryInterface)(REFIID riid, void ** ppvObject) {
        ...
        if ((riid == IID_IDispatch) ||
          (riid == __uuidof(device_event_interface))) {
 ...
 protected :
        ...
        // later to connect to the Connection Point of the
            // device_interface object.
        this -> QueryInterface(
          IID_IUnknown,
        ...
        if (pIUnknown) {
            ...
            pdevice_interface -> QueryInterface(
              IID_IConnectionPointContainer,
              reinterpret_cast<void**>(&pIConnectionPointContainerTemp));

            if (pIConnectionPointContainerTemp) {
                pIConnectionPointContainerTemp -> FindConnectionPoint(
                  __uuidof(device_event_interface),
                  &m_pIConnectionPoint);
